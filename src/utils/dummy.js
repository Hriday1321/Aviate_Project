export const dummy = [{
    name: 'Netflix',
    position: 'Junior SDE',
    moni: 28,
    applied: new Date('08-22-2022'),
    cur_date: new Date('09-17-2022'),
    cur_deadline: new Date('09-30-2022'),
    task: 'Improve efficiency of the first submitted project',
    timeline: [
        {
            task: 'Record an introductory video',
            sdate: new Date('08-29-2022'),
        },
        {
            task: 'Schedule Interview 1',
            sdate: new Date('09-06-2022'),
        }
    ]
},
{
    name: 'Amazon',
    position: 'SDE - 1',
    moni: 18,
    applied: new Date('08-24-2022'),
    cur_date: new Date('09-22-2022'),
    cur_deadline: new Date('10-02-2022'),
    task: 'Run backend service through Lambda functions',
    timeline: [
        {
            task: 'Record an introductory video',
            sdate: new Date('08-29-2022'),
        },
        {
            task: 'Schedule Interview 1',
            sdate: new Date('09-10-2022'),
        }
    ]
}]